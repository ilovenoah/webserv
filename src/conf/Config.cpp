...
Config::initSetterMap() {
	...
	srvSetterMap["alias"] = &Server::setAliasDirective;
	return srvSetterMap;
}
...
void Config::printServers() const {
	for (std::map<std::string, std::map<std::string, Server> >::const_iterator
		...
		for (std::map<std::string, Server>::const_iterator iter2 =
			...
			std::clog << "alias: " << iter2->second.getAliasDirective() << std::endl;
...
const Result<Server *, bool> Config::getDefaultServer(
	const std::string &listen) const {
	std::map<std::string, Server *>::const_iterator sviter =
		this->_defaultServers.find(listen);
	if (sviter == this->_defaultServers.end()) {
...
Server *Config::getServerPointer(const std::string &listen,
								 const std::string &hostName) {
	std::map<std::string, std::map<std::string, Server> >::iterator ipiter =
		this->_servers.find(listen);
	std::map<std::string, Server>::iterator sviter =
		ipiter->second.find(hostName);

	if (sviter == ipiter->second.end()) {
		return this->_defaultServers.find(listen)->second;
	}
