...
int getSocket(struct addrinfo *res) {
	...
	try {
		...
		if (sockfd == -1) {
			throw GenericException("socket creation failed.");
		}
		if (setNonBlocking(sockfd) == -1) {
			throw GenericException("Setting non-blocking failed.");
		}
		return sockfd;
...
void setSocketOption(int sockfd, struct addrinfo *res) {
	...
	try {
		if (setsockopt(sockfd, SOL_SOCKET, SO_REUSEADDR, &optval, sizeof(optval)) == -1) {
			throw GenericException("setsockopt failed.");
		}
	...
	try {
		for (size_t i = 0; i < servervec.size(); i++) {
			...
			if (getaddrinfo(servervec[i].getIpAddress().c_str(), servervec[i].getPort().c_str(), &hints, &res) != 0) {
				throw GenericException("getaddrinfo failed.");
			}
